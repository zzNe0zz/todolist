        ...
        try {
          ...
          let map = clone.filter(function(value,index){
           if(key.title && !key.date){
                      if(value.attributes.title.indexOf(`${key.title}`)=== -1){
                        return
                      }
                      else  if(value.attributes.title.indexOf(`${key.title}`)!== -1){                       
                       return value
                      }                     
                    }
...
// console.log(reg.test(`${keySearch.search.slice(1)}`));
 ...
 await deleData(dataIP.id,token)
  ...
  return (
    <>
    {
      dataMap.length<1?<NothingTodo></NothingTodo>:<div style={{position:"relative"}}>
      <div className='card-body'>
      {
          dataMap.map(function(value,index){
           return(
            ...
            <Pagination defaultCurrent={1} total={data.length}  pageSize={4} />
            </div>
